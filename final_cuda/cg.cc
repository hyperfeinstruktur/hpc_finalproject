...
The mat_vec function calls a kernel (see matrix_coo_gpu.cu) and does the computation on the GPU.
To do this, two arrays (p and Ap) need to be copied back and forth between host and device.
The rest of the code is the same as in the original sequential one.
...
void CGSolverSparse::solve(std::vector<double> & xvect,const dim3 & grid_size,const dim3 & block_size) {
  ...
  // r = b - A * x;
  // Here I use p to store x to avoid having to allocate extra device memory for this
  // single computation
